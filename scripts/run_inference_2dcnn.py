#!/usr/bin/env python3
"""Run 2D-CNN Binary Kelp Detection Inference."""
...
def main():
    parser = argparse.ArgumentParser(
        description='Run 2D-CNN binary kelp detection on Sentinel-2 L1C scene'
    )
    parser.add_argument('--scene', '-s', required=True,
                        help='Path to Sentinel-2 L1C .SAFE directory')
    parser.add_argument('--output', '-o', required=True,
                        help='Output kelp classification GeoTIFF path')
    parser.add_argument('--model', default='models/2dcnn_binary_int8.tflite',
                        help='Path to TFLite model (default: models/2dcnn_binary_int8.tflite)')
    parser.add_argument('--scaler', default='models/scaler_2dcnn_binary.joblib',
                        help='Path to scaler file (default: models/scaler_2dcnn_binary.joblib)')
    ...
    preprocessor = Preprocessor(model_type='2dcnn', scaler_path=args.scaler)
    data_ready = preprocessor.prepare_for_inference(data, roi_mask=roi_mask)
    print(f"  Preprocessed shape: {data_ready.shape}")

    print("Running 2D-CNN binary inference...")
    predictor = KelpPredictor(args.model)
    classification_roi = predictor.predict_and_classify(
        data_ready, batch_size=args.batch_size, threshold=args.threshold
    )

    n_pixels = metadata['height'] * metadata['width']
    if roi_mask is not None:
        roi_flat = roi_mask.flatten()
        classification_1d = np.zeros(n_pixels, dtype=np.uint8)
        classification_1d[roi_flat] = classification_roi
    else:
        classification_1d = classification_roi
